...
SYSTEMATIC_COEFFICIENTS = [0.3,1,3,10,15,20]
...
def noise(Z_complex_gt, systematic_coeff, freqs):
    """
    Introduce un errore sistematico deterministico in eccesso
    Sovrastima il modulo di +X% e trasla la fase di +X gradi per tutta la curva.
    
    """
    Z_abs = np.abs(Z_complex_gt)
    Z_phase = np.angle(Z_complex_gt)
    
    # coeff errore in perc err di fase in radianti
    err_abs_fisso = systematic_coeff / 100.0
    err_phase_fisso = systematic_coeff * (np.pi / 180.0)
    
    # somma in modulo 
    Z_abs_noisy = Z_abs * (1 + err_abs_fisso)
    
    # somma in fase
    Z_phase_noisy = Z_phase + err_phase_fisso
    
    Z_complex_noisy = Z_abs_noisy * np.exp(1j * Z_phase_noisy)
    
    return Z_complex_noisy

def scatter_plot(df, df_noisy, sys_err_coeff):
    ...
    plt.xlabel("Z' (Ohm)")
    plt.ylabel("-Z'' (Ohm)")
    plt.title(f"Comparative Nyquist Scatter - Temp {temp}°C - Systematic Error Coefficient: {sys_err_coeff}")
    plt.grid(True)
    plt.legend()
    #plt.show()
    plt.savefig('./figures/scatter_plot_{}_sys_err_coefficient.png'.format(str(sys_err_coeff).replace('.', '_')))
    plt.close()
...
def main():
    ...
    for sys_err_coeff in SYSTEMATIC_COEFFICIENTS:
        
        tag = f"{sys_err_coeff}".replace('.', '_')
        out_path = f"./dataset/dataset_all_noisy_{tag}.csv"
        
        print(f"\n--- Systematic error coefficient: {sys_err_coeff} ---")
    
        if os.path.exists(out_path):
            df_noisy = pd.read_csv(out_path)
            scatter_plot(df, df_noisy, sys_err_coeff)
            continue
        ...
        for i in range(len(df)):
            ...
            # introduco il rumore
            Z_complex_noisy = noise(Z_complex_gt,sys_err_coeff,freqs)
        ...
        plot_nyquist(df_noisy, ax, f"NOISY DATA - systematic error coefficient: {sys_err_coeff}", samples_per_temp=127)
