...
param_grid = {
    ...
    'elasticnet__l1_ratio': np.linspace(0.1, 1, 100)
}
...
grid.fit(X, y)
print("Best alpha:", grid.best_estimator_.get_params()['elasticnet__alpha'])
...
for fold, (train_idx, test_idx) in enumerate(CVGenerator(df)):
    ...
    print(f"  R2 = {r2}")
    print(f"  Best alpha = {grid.best_estimator_.get_params()['elasticnet__alpha']}")
