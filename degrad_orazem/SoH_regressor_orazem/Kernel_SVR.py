...
# ========================================
# MODELLO + BAYES SEARCH
# ========================================
pipeline = Pipeline([
    ('scaler', PartialScaler(n_features_to_scale=X_eis_len)),
    ('KernelSVR', SVR(kernel=CompositeKernel()))
])

param_grid = {
    'KernelSVR__C': Real(1, 100, prior='log-uniform'),
    'KernelSVR__epsilon': Real(0.001, 0.1, prior='log-uniform'),
    'KernelSVR__kernel__lengthscale_eis': Real(1.0, 50.0, prior='log-uniform'),
    'KernelSVR__kernel__variance_eis': Real(1.0, 50.0, prior='log-uniform'),
    'KernelSVR__kernel__lengthscale_temp': Real(1e-5, 1e-2, prior='log-uniform'),
    'KernelSVR__kernel__variance_temp': Real(0.1, 10.0, prior='log-uniform'),
    'KernelSVR__kernel__variance_soc': Real(1e-7, 1e-3, prior='log-uniform')
}
...
for fold, (train_idx, test_idx) in enumerate(CVGenerator(df)):
    ...
    y_pred = pipeline.predict(X_test) + y_train_mean
    ...
    print(f"  R2 = {r2}")
    print(f"  Kernel: {pipeline.named_steps['KernelSVR'].kernel}")
    print(f"  C: {pipeline.named_steps['KernelSVR'].C}")
    print(f"  n_support: {pipeline.named_steps['KernelSVR'].n_support_}")
